...
parser.add_argument('--BF', type=int, default=1, help='Total iterations')
...
class SEPSO_UP():
    def __init__(self, opt):
        ...
        self.Exp_Name = f'SEPSO_BF{opt.BF}_N{opt.N}_T{opt.Total_iterations}_bs{opt.Bootstrap}' + timenow
    ...
    def iterate(self):
        sepso_down = SEPSO_Down(self.BF)
        '''双for循环遍历计算SEPSO_UP所有粒子的Fitness'''
        for i in range(self.Total_iterations):
            t0 = time.time()
            for Gid in range(self.G):
                for Nid in range(self.N):
                    self.fitness[Gid,Nid] = sepso_down.evaluate_params( self.Locate[1,Gid,Nid].clone() )
            ...
            print(f'BF:{self.BF},  Iteration:{i},  logMinvalue:{round(torch.log10(self.Tbest_value).item(),1)}, '
                  f'IterationTime:{time_per_iteration}s,   RemainTime:{round((self.Total_iterations-i)*time_per_iteration/3600,1)}h')

            if self.write:
                self.writer.add_scalar('LogTbest', torch.log10(self.Tbest_value), global_step=i)
                self.writer.add_scalar('MeanFit', torch.log10(self.fitness.mean()), global_step=i)
